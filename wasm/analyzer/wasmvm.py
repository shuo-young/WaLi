        ...
        for i in path_instrs:
            ...
            if i.name in ["unreachable", "nop", "block", "loop", "else", "end", "br"]:
                ...
                if "$L" + str(i.operand_interpretation.split(" ")[-1]) in locals:
                    obj = locals["$L" +
                                 str(i.operand_interpretation.split(" ")[-1])]
                    item = StackItem()
                ...
                locals["$L" + str(i.operand_interpretation.split(" ")
                                  [-1])] = self.stack.pop()
                ...
                self.stack.append(
                    self.globals["$G" + str(i.operand_interpretation.split(" ")[-1])])
            elif i.name == "set_global":
                self.globals["$G" +
                             str(i.operand_interpretation.split(" ")[-1])] = self.stack.pop()
                ...
                if (int(i.operand_interpretation.split(" ")[-1]) + int(addr_operand)) in self.mem_tb:
                    ...
                    if (int(i.operand_interpretation.split(" ")[-1]) + int(addr_operand)) in self.caller_memory:
                        ...
                        if(self.focus == True):
                            print("load getCaller memory value at", int(
                                i.operand_interpretation.split(" ")[-1]) + int(addr_operand))
                        ...
                        val = self.mem_tb[int(i.operand_interpretation.split(
                            " ")[-1]) + int(addr_operand)]
                        val = unpack('i', val)[0]
                ...
                addr_operand = self.stack.pop()
                ef_addr1 = int(i.operand_interpretation.split(
                    " ")[-1]) + int(addr_operand)
                ef_addr2 = (
                    4 + int(i.operand_interpretation.split(" ")[-1])) + int(addr_operand)
                ...
                if (val == CALLER_SYSMBOL):
                    self.caller_memory.append(
                        int(i.operand_interpretation.split(" ")[-1]) + int(addr_operand))
                    print("getCaller value store to memory: " +
                          " ".join('%s' % id for id in self.caller_memory))
                val = pack('i', int(val))
                self.mem_tb[int(i.operand_interpretation.split(
                    " ")[-1]) + int(addr_operand)] = val
            elif i.name == "i64.store":  # TODO: 64bit
                val = self.stack.pop()
                val = pack('q', int(val))
                addr_operand = self.stack.pop()
                self.mem_tb[int(i.operand_interpretation.split(
                    " ")[-1]) + int(addr_operand)] = val[:4]
                self.mem_tb[int(i.operand_interpretation.split(
                    " ")[-1]) + int(addr_operand) + 4] = val[4:]
                ...
                if self.func_map[f_idx][3] != "import":
                    # not FBEI function
                    pops_n = len(
                        list(filter(None, self.func_map[f_idx][1].split(" "))))
                    pushs_n = len(
                        list(filter(None, self.func_map[f_idx][2].split(" "))))
                    ...
                    if recursion:
                        ...
                        try:
                            indirect_target = unpack(
                                "i", self.mem_tb[int(args_next[0])])[0]
                            ...
                            if not f_pts[indirect_target - start_idx] in self.indirect_targets:
                                self.indirect_targets.append(
                                    f_pts[indirect_target - start_idx])
                        except:
