...
def Vuldetect(file_name, vul_type, output_file):
    ...
    if vul_type == "1":
        ...
        getCallerOffset, setStorageOffset = get_call_offsets(
            func_main_edges, func_main_blocks, focus_funcs_get_caller, focus_funcs_storage)
        print("")
        print("getCallerOffset: " + " ".join('%s' %
              id for id in getCallerOffset))
        print("setStorageOffset: " + " ".join('%s' %
              id for id in setStorageOffset))
        paths_blocks = get_call_paths(
            cfg, func_main_blocks, getCallerOffset, setStorageOffset)
